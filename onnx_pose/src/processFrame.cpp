...
vector<pair<int, int>> segment = {
        ...
        make_pair(11, 13), make_pair(13, 15), 
        make_pair(11, 12), make_pair(11, 23), make_pair(23, 24), make_pair(24, 12),
        make_pair(24, 26), make_pair(26, 28), 
        make_pair(23, 25), make_pair(25, 27) };
...
int32_t processFrame::Process(cv::Mat& frame) // details of analyzing the image pose processes
{
   ...
   tuple<Mat, Mat, Mat, Mat, Mat, float> temp=pose.at(0);
    ...
    // }
       ...
       for (size_t i{0}; i < indeces.size(); ++i) {
        if(!(indeces.at(i)==8) && !(indeces.at(i)==7)){
    ...
    for(auto p:segment){
     line(frame, Point(landmarksScreen.at<float>(p.first, 0), landmarksScreen.at<float>(p.first, 1)),Point(landmarksScreen.at<float>(p.second, 0), landmarksScreen.at<float>(p.second, 1)), Scalar(255, 255, 255), 2);
     }
    //add normalized
    ...
    if (mode == "send") {
        ...
        while(true){
                ...
                if (activity < 0) {
                    ...
                    else if(strncmp(buffer,"EXIT",1024) ==0){
                        int pid = getpid();
                        kill(pid , SIGINT);
                    }
    ...
    resizeWindow("Display frame", 650, 970);
    moveWindow("Display frame", 1345, 42);
...
void processFrame::connect_server(){
  ...
  // Listen for connections
    ...
    std::cout << "From pose: Server listening on port ..." << std::endl;
  ...
  // Accept a connection
    ...
    std::cout << "From pose: Client connected from " << inet_ntoa(clientAddress.sin_addr) << ":" << ntohs(clientAddress.sin_port) << std::endl;
