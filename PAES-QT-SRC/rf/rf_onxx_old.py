...
global_class_object = None

def signal_handler(sig, a):
    print("Signal received, shutting down python process...")
    if global_client_pos:
        #global_client_pos.sendall("EXIT".encode(FORMAT))
        global_client_pos.close()

    if global_client_dscn:
        global_client_dscn.sendall("EXIT".encode(FORMAT))  
        global_client_dscn.close()

    if global_class_object:
        global_class_object.put("EXIT")
...
class MyRandomForest():
    def __init__(self):
        ...
        global global_class_object
        global_class_object = self.log_queue
    ...
    def connect_dscn(self):
        while True:
            try:
                ...
                self.log_message("Connected by dscn")
                break
            except Exception as e:
                time.sleep(2)
                self.log_message(f"Error in dscn: {e}")
    ...
    def main(self):
        ...
        global global_client_dscn, global_client_pos
                ...
                try:
                    ...
                    if(top_3_probabilities[0]>moves_tresholds[top_3_predictions[0]]):
                        ...
                        #prediction mmove same current move and ,current move is done 
                            if(  not current_is_done and current_class == top_3_predictions[0] and time.time()-current_time>move_times[current_class] and current_class!="walk"):#
                            ...
                            elif (current_class == top_3_predictions[0] and time.time()-current_time<move_times[current_class] and current_class!="walk"):
                            ...
                            elif(current_is_done and current_class == top_3_predictions[0]  and time.time()-current_time>move_times[current_class] and current_class!="walk"):
                            ...
                            if ( top_3_predictions[0]!=current_class):
                                if (new_move["name"]==top_3_predictions[0] and new_move["time"]>move_times[new_move["name"]]/2 and new_move['name']!="walk"):
                                    ...
                                    self.client_dscn.sendall(f"{current_class} {top_3_probabilities[0]:.4f} new_move 0".encode(FORMAT))  
